"""chaupalma_csv_pipeline.py - CSV ETVL pipeline. Using World Happiness Report data.
...
def load_stats_report(*, stats: dict[str, float], out_path: Path) -> None:
    ...
    with out_path.open("w", encoding="utf-8") as f:
        f.write("World Perceptions of Corruption Score Statistics\n")
...
def run_pipeline(*, raw_dir: Path, processed_dir: Path, logger: Any) -> None:
    ...
    output_file = processed_dir / "chaupalma_corruption_stat.txt"

    # E
    scores = extract_csv_scores(file_path=input_file, column_name="Perceptions of corruption")
